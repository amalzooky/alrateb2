                    ...
                    <div class="row breadcrumbs-top">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href=""> المحاضرات </a>
                                </li>
                                <li class="breadcrumb-item active"> تعديل - {{$lectures[0]->lecture_name}}
                                </li>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title" id="basic-layout-form"> تعديل محاضره </h4>
                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body">
                                        <form class="form" action="{{route('admin.lectures.update', $lectures[0]->id)}}"
                                              method="POST"
                                                ...
                                                @if(get_languages() -> count() > 0)
                                                    @foreach(get_languages() as $index => $lang)
                                                        @foreach($lectures as $lecture)
                                                            @if($lecture->translation_lang === $lang->abbr)
                                                                <div class="row">
                                                                    <div class="col-md-12">
                                                                        <div class="form-group">
                                                                            <label for="projectinput1"> اسم المحاضره
                                                                                - {{__('messages.'.$lang -> abbr)}} </label>
                                                                            <input type="text" value="{{$lecture->lecture_name}}" id="name"
                                                                                   class="form-control"
                                                                                   placeholder="  "
                                                                    ...
                                                                    <div class="col-md-12">
                                                                        <div class="form-group">
                                                                            ...
                                                                            <textarea class="form-control"
                                                                                      placeholder="  "
                                                                                      name="lecture[{{$index}}][description]">{{$lecture->lecture_description}}</textarea>
                                                                ...
                                                                <select name="lecture[0][subject]" class="form-control">
                                                                    ...
                                                                    @if(!empty($subjects) && count($subjects) > 0)
                                                                        @foreach($subjects as $subject)
                                                                            <option value="{{$subject->id}}" {{$subject->id === $lectures[0]->lecture_subject ? 'selected' : ''}}>{{$subject->subject_name}}</option>
                                                                        @endforeach
                                                                    @else
                                                                        <option value="" disabled>ليس هناك مواد</option>
                                                                    @endif
                                                                </select>
                                                                @if(!empty(session()->get('errors')['lecture.0.subject']))
                                                                    <span class="text-danger"> هذا الحقل مطلوب</span>
                                                                @endif
                                                            </div>
                                                        </div>
                                                                ...
                                                                <select name="lecture[0][teacher]" class="form-control">
                                                                    ...
                                                                    @if(!empty($teachers) && count($teachers) > 0)
                                                                        @foreach($teachers as $teacher)
                                                                            <option value="{{$teacher->id}}" {{$teacher->id === $lectures[0]->lecture_teacher ? 'selected' : ''}}>{{$teacher->fullname}}</option>
                                                                        @endforeach
                                                                    @else
                                                                        <option value="" disabled>ليس معلمين</option>
                                                                    @endif
                                                                </select>
                                                                @if(!empty(session()->get('errors')['lecture.0.teacher']))
                                                                    <span class="text-danger"> هذا الحقل مطلوب</span>
                                                                @endif
                                                            </div>
                                                        </div>
